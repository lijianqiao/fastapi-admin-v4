...
> **项目名称**: fastapi-admin
> **文档版本**: v1.2（代码审查修复：RBAC 委派、分层与可观测性）
> **撰写人**: 李剑桥（架构师）
> **日期**: 2026-09-25
> **基于**: PRD v1.0
...
#### 1.4 v1.2 补充：分层、RBAC 委派与可观测性

**后端分层（v1.2 起）**

| 层 | 目录 | 职责 |
|---|---|---|
| 路由 | `app/api/v1/` | 只做 HTTP 适配：参数、响应模型、状态码、审计提交 |
| 用例 | `app/services/` | 业务规则、行锁、会话撤销、密码哈希；只 flush 不 commit |
| 仓储 | `app/crud/` | 查询；`CRUDBase` 提供分页与白名单更新，`SoftDeleteCRUD` 提供回收站 |
| 横切 | `app/core/` | 配置、依赖（`DbSession`、`PageQuery`、`Audit`/`audited()`）、领域异常、权限注册表、中间件 |

- 领域失败抛 `app.core.errors.AppError` 子类，由单一处理器渲染为统一信封。
- 回收站接口由 `app/api/v1/recycle_bin.py` 的工厂生成。
- 新增资源时，按上表分层实现，并参考"权限注册表"一节新增权限。

**权限注册表**

- `app/core/permissions.py` 的 `Perm` 是权限码的唯一来源。路由只能 `require_permission(Perm.X)`。
- `init_db.py` 按注册表同步 `permissions` 表，并把这些行标记为 `is_system`。
- 系统权限的 `code` 不可修改，也不可删除或永久删除。所有权限的 `code` 创建后都不可修改。
- 测试会校验：注册表、路由实际使用的权限、前端 `PERMISSIONS` 常量三者一致。

**RBAC 委派规则（非超级管理员）**

1. 不能修改超级管理员账户（更新、停用、删除、重置密码、分配角色、恢复、永久删除）。
2. 不能修改自己的角色。
3. 新授予的权限必须是自己当前有效权限的子集。"授予"包括：给用户新增角色、给角色新增权限、启用或恢复仍有在用用户的角色、启用或恢复挂在未删除角色上的权限。

**会话与审计**

- 会话族除滑动过期外，还有绝对有效期 `absolute_expires_at`（`REFRESH_SESSION_ABSOLUTE_LIFETIME_DAYS`，默认 30 天）。
- 审计表不设外键，写入时快照操作人用户名（`actor_username`），永久删除用户不会改写审计记录。
- 审计列表的总数封顶为 10000，超过部分以"10000+"显示，并拒绝更深的翻页。
- 前端用 Web Locks 让各标签页的 refresh 调用串行执行，避免触发服务端的重放检测。

**可观测性**

- `RequestIdMiddleware` 接受合法的入站 `X-Request-ID`，否则生成一个新的；该 ID 会回写到响应头并写入每条日志。
- `/health` 为存活探针，`/health/ready` 为数据库就绪探针。
